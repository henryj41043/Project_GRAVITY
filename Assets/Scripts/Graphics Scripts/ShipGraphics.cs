...
public class ShipGraphics : MonoBehaviour
{
    ...
    public void Draw(Quaternion preRotation, Quaternion postRotation, float verTranslation)
    {
        ...
        RightLight.intensity = Mathf.Max(RightLight.intensity, -Mathf.Clamp(hOrientation * 4, -1.0f, 0.0f));
        RightBooster.startLifetime = Mathf.Max(RightBooster.startLifetime, -Mathf.Clamp(hOrientation * 4, -1.0f, 0.0f));

        LeftLight.intensity = 8.0f * Mathf.Clamp(LeftLight.intensity, 0.0f, 1.0f);
        LeftBooster.startLifetime = 3.0f * Mathf.Clamp(LeftBooster.startLifetime, 0.0f, 1.0f);
        RightLight.intensity = 8.0f * Mathf.Clamp(RightLight.intensity, 0.0f, 1.0f);
        RightBooster.startLifetime = 3.0f * Mathf.Clamp(RightBooster.startLifetime, 0.0f, 1.0f);

        MainLight.intensity = 8.0f * verTranslation;
        MainThruster.startLifetime = 0.1f * verTranslation;
	}
}
